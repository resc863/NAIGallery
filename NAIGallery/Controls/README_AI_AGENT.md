...
**Behavior:**
- If `Height` is set → computes `Width = Height * AspectRatio`
- If `Width` is set → computes `Height = Width / AspectRatio`
- If both set → uses both directly
- If neither set → uses available size heuristics
